...
def refreshData(tabBody:dict,Obj,length):
  """刷新表数据"""
...
class CreateTable():
  ...
  def __init__(self,parent:Frame):
    ...
    cavBody=Canvas(ftable,yscrollcommand=self.scro.set)
    ftHead,ftBody=Frame(ftable),Frame(cavBody)
    frame.pack(side='left',anchor='nw'),ftable.pack(side='left'),ftHead.pack(),cavBody.pack(),ftBody.pack(),self.scro.pack(side='right',fill='y')
    ...
    def refreshTable():
      ...
      for _ in range(comp):
        if len(self.tabBody)>len(Obj):# 减少          
          ...
          i=len(self.tabBody)
          self.addRow(ftBody,i)
          self.select.update({'tabBody':self.tabBody})
        
      refreshData(self.select['tabBody'],Obj,len(self.txt))
      
      parent.after(1000,refreshTable)
...
# ---------------------------------------------------------------------------------------------------------------------
  ...
  def addRow(self,parent:Frame,i:int):
    """添加一行控件[框架]"""
    LabelFrame_=dict()
    rD=LabelFrame(parent,name=f'rD_{i}',border=5,borderwidth=1,bg=self.bgcolor,fg=self.fgcolor)
